		...
		for (j = 0; risks[j] != NULL; j++) {
			...
			for (k = 0; k < channels->len; k++) {
				...
				if (strcmp (snapd_channel_get_name (channel), name) == 0) {
					version = g_strdup_printf ("%s (%s)", snapd_channel_get_version (channel), snapd_channel_get_revision (channel));
					break;
				}
			}
			if (version == NULL)
			...
			for (k = 0; k < channels->len; k++) {
				...
				if (snapd_channel_get_branch (c) != NULL &&
					...
					g_autofree gchar *v = g_strdup_printf ("%s (%s)", snapd_channel_get_version (c), snapd_channel_get_revision (c));
					add_channel (app, snapd_channel_get_name (c), v, tracking_channel);
				}
			}
		      ...
		      GError **error)
{
	...
	gs_app_set_state (app, AS_APP_STATE_INSTALLING);
	if (!snapd_client_refresh_sync (client, gs_app_get_id (app), NULL, progress_cb, app, cancellable, error)) {
		gs_app_set_state_recover (app);
		snapd_error_convert (error);
		return FALSE;
	}
